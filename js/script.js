(function () {
  ...
  const btn = e => {
    ...
    if (
      ...
      if (n2 && lastop) {
        result = ((parseFloat(n2) % 1 !== 0) && (parseFloat(result) % 1 !== 0)) ? (cal((parseFloat(n2) * 10), (parseFloat(result) * 10), cals[lastop]) / 10) : cal(parseFloat(n2), parseFloat(result), cals[lastop])
      }
